...
def save_data():
    ...
    previous_data = None
    try:
        previous_data = pd.read_pickle('job_listings.pkl')
    except:
        pass

    # creating a dataframe from the job_listings list
    df = pd.DataFrame([vars(job_listing) for job_listing in job_listings])
    ...
    df.to_pickle('job_listings.pkl')

    # emptying the job_listings list
    job_listings.clear()
...
# taking the auth key from the auth_key.txt file
